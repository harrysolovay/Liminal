...
const dracula = await Deno.readTextFile(
  new URL("dracula.txt", import.meta.url),
)
const SAMPLE_LENGTH = 1000

const PassagePoem = L.thread(async function*() {
  ...
  const _summary = yield* L.string

  yield "Use that summary to create a poem."
}, L.string)

const poem = await PassagePoem.run(model(new OpenAI(), "gpt-4o-mini"))
console.log(poem)
